        ...
        with st.form("trip_form"):
            ...
            if submitted:
                if not departure_place or not arrival_place or not trip_purpose:
                    ...
                    # 先将所有现有行程赋值给TripRecorder实例
                    trip_recorder.trips = st.session_state.trips.copy()
                    # 使用call方法记录此次行程（这不会影响已有行程）
                    recorder_result = trip_recorder.call(json.dumps({
                        "trip_info": trip_info
                    }))
                    
                    # 解析结果但不覆盖已有的行程列表
                    try:
                        result = json.loads(recorder_result)
                        # 记录工具调用的成功状态但不重新赋值trips
                        success = result.get("status") == "success"
                        if not success:
                            st.warning(f"行程记录工具返回警告: {result.get('message', '未知警告')}")
                    except Exception as e:
                        st.error(f"处理行程数据时出错: {str(e)}")
